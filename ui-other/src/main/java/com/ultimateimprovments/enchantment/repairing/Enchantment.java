...
/**
 * Repairing — real datapack enchantment with a PDC failsafe.
 * <p>
 ...
 *   <li><b>Datapack crashed:</b> {@link #getLevel} falls back to PDC, so the repair
 *       keeps working even if the datapack dies;</li>
...
public final class Enchantment {
    ...
    public static final NamespacedKey ENCHANTMENT_KEY = new NamespacedKey("ui", "repairing");

    /** PDC mirror key: {@code ui:repairing_level} (backup copy of the enchantment level). */
    public static final NamespacedKey LEVEL_KEY = new NamespacedKey(Main.getInstance(), "repairing_level");

    /** PDC key: {@code ui:repairing_last_repair} — epoch millis of the last repair (cooldown). */
    ...
    /**
     * Returns the Repairing level on the given item.
     * <p>
    ...
    /**
     * Sets the Repairing level on the given item.
     * <p>
    ...
    /**
     * Removes the Repairing enchantment from the given item (real enchantment and PDC mirror).
     *
     * @param item the item to modify
     */
    ...
    /**
     * Checks if the given item has the Repairing enchantment.
     */
    public static boolean hasRepairing(@NotNull ItemStack item) {
    ...
    public static long getLastRepairMillis(@NotNull ItemStack item) {
        if (!item.hasItemMeta()) return 0L;
        ItemMeta meta = item.getItemMeta();
        if (meta == null) return 0L;
        Long value = meta.getPersistentDataContainer().get(LAST_REPAIR_KEY, PersistentDataType.LONG);
        return value != null ? value : 0L;
    }

    /** Stores the last-repair timestamp (epoch millis). */
    public static void setLastRepairMillis(@NotNull ItemStack item, long millis) {
        ItemMeta meta = item.getItemMeta();
        if (meta == null) return;
        meta.getPersistentDataContainer().set(LAST_REPAIR_KEY, PersistentDataType.LONG, millis);
    ...
    /**
     * True if the stack carries the Repairing enchantment (non-air + level &gt; 0).
     * Used by the repair sweep.
     */
    public static boolean isRepairing(@Nullable ItemStack item) {
        return item != null && item.getType() != Material.AIR && getLevel(item) > 0;
    }

    /**
     * The enchantment works on ANY item of the Integrity system — i.e. anything with
     * a durability bar. Uses {@link IntegrityManager#getMaxDurability} (component
