...
/**
 * AOEEnchantmentSyncListener — detects items carrying the AoE enchantment and
 * keeps the PDC mirror ({@code ui:aoe_level}) in sync with the real enchantment.
 * <p>
 * This is the failsafe backbone: whenever an item with the charm is seen, its
 * level is mirrored into PDC, so a datapack crash never loses the level. When
 * the datapack comes back, PDC-only items get the real charm re-applied.
 * <p>
 * Sync triggers:
 * <ul>
 ...
 * </ul>
 */
public class AOEEnchantmentSyncListener implements Listener {
    ...
    public void onPickup(EntityPickupItemEvent event) {
        ...
        AOEEnchantment.syncItem(event.getItem().getItemStack());
    }
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (event.getInventory() instanceof GrindstoneInventory
            ...
            AOEEnchantment.clearPdcMirror(event.getCurrentItem());
            return;
        }

        // Mirror-only: safe to call, no stack mutation in the re-apply direction.
        AOEEnchantment.mirrorItem(event.getCurrentItem());
        AOEEnchantment.mirrorItem(event.getCursor());
    }
    ...
    public void onDrag(InventoryDragEvent event) {
        ...
        for (ItemStack item : event.getNewItems().values()) {
            AOEEnchantment.mirrorItem(item);
        }
    }
    ...
    public static void syncInventory(Player player) {
        ...
        for (ItemStack item : inv.getStorageContents()) {
            AOEEnchantment.syncItem(item);
        }
        // Armor + offhand
        for (ItemStack item : inv.getArmorContents()) {
            AOEEnchantment.syncItem(item);
        }
        AOEEnchantment.syncItem(inv.getItemInOffHand());
        AOEEnchantment.syncItem(inv.getItemInMainHand());
        // Cursor (open inventory screen)
        AOEEnchantment.syncItem(player.getOpenInventory().getCursor());
    }
    ...
    public static void register(Main plugin) {
        Bukkit.getPluginManager().registerEvents(new AOEEnchantmentSyncListener(), plugin);
        Bukkit.getScheduler().runTaskTimer(plugin, AOEEnchantmentSyncListener::sweepAllPlayers,
                SCAN_INTERVAL_TICKS, SCAN_INTERVAL_TICKS);
        ConsoleLogger.info("[AoE] Sync listener registered (PDC failsafe, scan every "
