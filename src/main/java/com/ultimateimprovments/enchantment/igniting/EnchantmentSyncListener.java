...
/**
 * EnchantmentSyncListener — detects armor carrying the Igniting enchantment and
 * keeps the PDC mirror ({@code ui:igniting_level}) in sync with the real enchantment.
 * <p>
 * This is the failsafe backbone: whenever an item with the charm is seen, its
 * level is mirrored into PDC, so a datapack crash never loses the charm. When the
 * datapack comes back, PDC-only items get the real charm re-applied.
 * <p>
 * Sync triggers:
 * <ul>
...
public class EnchantmentSyncListener implements Listener {
    ...
    public static void register(Main plugin) {
        ...
        ConsoleLogger.info("[Igniting] Sync listener registered (PDC failsafe, scan every "
