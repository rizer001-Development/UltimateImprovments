 ...
 * <p>
 * <b>Failsafe design (same as AoE/AutoSmelt):</b> every item carrying the charm ALSO stores the
 * level (always 1) in the {@code ui:veinminer_level} PDC key — a backup mirror:
 * <ul>
 ...
 * </ul>
 * <p>
 * Effect: when a player mines an ore with a VeinMiner pickaxe, the ENTIRE connected vein of the
 * same ore type is broken at once (raw ore blocks), consuming tool integrity per block.
 * Sneaking disables VeinMiner for precise single-block mining.
 * <p>
 * Max level: 1<br>
 * Works on: pickaxe
 */
public final class Enchantment {
    ...
    /**
     * Returns whether the given tool has the VeinMiner enchantment.
    ...
    /**
     * Checks if the given tool has the VeinMiner enchantment.
     */
    public static boolean hasVeinMiner(@NotNull ItemStack item) {
        return getLevel(item) > 0;
    }

    /**
     * Sets the VeinMiner enchantment on the given tool.
    ...
    /**
     * Removes the VeinMiner enchantment from the given tool (real + PDC mirror).
     *
     * @param item the tool to modify
     */
    ...
    /**
     * Checks if the item type can accept the VeinMiner enchantment.
     */
    ...
    /**
     * Checks if the material can accept the VeinMiner enchantment.
     */
