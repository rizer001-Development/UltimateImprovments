 ...
 * <p>
 * <b>Failsafe design (same as AoE):</b> every item carrying the charm ALSO stores the
 * level (always 1) in the {@code ui:autosmelt_level} PDC key — a backup mirror:
 * <ul>
 ...
 * </ul>
 * <p>
 * Effect: when a player mines a block with an AutoSmelt tool, the block drops are
 * smelted (if the drop has a furnace recipe) — raw ore becomes ingots, sand becomes
 * glass, etc. Nothing drops → nothing is smelted.
 * <p>
 * Max level: 1<br>
...
public final class Enchantment {
    ...
    /**
     * Returns whether the given tool has the AutoSmelt enchantment.
    ...
    /**
     * Checks if the given tool has the AutoSmelt enchantment.
     */
    public static boolean hasAutoSmelt(@NotNull ItemStack item) {
        return getLevel(item) > 0;
    }

    /**
     * Sets the AutoSmelt enchantment on the given tool.
    ...
    /**
     * Removes the AutoSmelt enchantment from the given tool (real + PDC mirror).
     *
     * @param item the tool to modify
     */
    ...
    /**
     * Checks if the item type can accept the AutoSmelt enchantment.
     */
    ...
    /**
     * Checks if the material can accept the AutoSmelt enchantment.
     */
