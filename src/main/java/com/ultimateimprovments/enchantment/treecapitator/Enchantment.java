...
/**
 * TreeCapitator enchantment — real datapack enchantment with a PDC failsafe.
 * <p>
 * Registers {@code minecraft:treecapitator} (file {@code data/minecraft/enchantment/treecapitator.json})
 * as a REAL data-driven enchantment: glint, description, anvil &amp; book compatibility,
 * {@code /enchant} support. Has exactly ONE level.
 * <p>
 * <b>Failsafe design (same as AoE/AutoSmelt/VeinMiner):</b> every item carrying the charm ALSO
 * stores the level (always 1) in the {@code ui:treecapitator_level} PDC key — a backup mirror:
 * <ul>
 ...
 * </ul>
 * <p>
 * Effect: when a player breaks a log with a TreeCapitator axe, the ENTIRE connected trunk
 * (and branches) of the tree is broken at once, consuming tool integrity per log.
 * Sneaking disables TreeCapitator for precise single-block cutting.
 * <p>
 * Max level: 1<br>
 * Works on: axe
 */
public final class Enchantment {
    ...
    public static final NamespacedKey ENCHANTMENT_KEY = NamespacedKey.minecraft("treecapitator");

    /** PDC mirror key: {@code ui:treecapitator_level} (backup copy of the enchantment level). */
    public static final NamespacedKey LEVEL_KEY = new NamespacedKey(Main.getInstance(), "treecapitator_level");
    ...
    /**
     * Returns whether the given tool has the TreeCapitator enchantment.
    ...
    /**
     * Checks if the given tool has the TreeCapitator enchantment.
     */
    public static boolean hasTreeCapitator(@NotNull ItemStack item) {
        return getLevel(item) > 0;
    }

    /**
     * Sets the TreeCapitator enchantment on the given tool.
    ...
    /**
     * Removes the TreeCapitator enchantment from the given tool (real + PDC mirror).
     *
     * @param item the tool to modify
     */
    ...
    /**
     * Checks if the item type can accept the TreeCapitator enchantment.
     */
    ...
    /**
     * Checks if the material can accept the TreeCapitator enchantment.
     */
